...
const int STASHCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString STASHCOIN_IPC_PREFIX("stashcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(STASHCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(STASHCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(STASHCOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click stashcoin: links)
...
{
    // clicking on stashcoin: URLs creates FileOpen events on the Mac:
