...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non STA unit input in lnReqAmount, we generate the URI with STA as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(StashcoinUnits::format(StashcoinUnits::STA, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
